...
class File {
    public function __construct( public $name, public $size)
    {
    }
};
...
class Directory {
    ...
    public function getSizeAttribute()
    {
        ...
        $dirsSize = $this->directories->map(fn($d)=>$d->size)->sum();
        $total = $filesSize + $dirsSize;
        return $total;
    }
...
function parseInput($input)
{
    ...
    foreach ($lines as $line) {
        $line = str($line);
        $parts = collect(explode(' ', $line));
        if ($parts[0] == $commandIndicator) {
            $mode = 'command';
            $parts->shift();
            $command = $parts->shift();
            $args = $parts;
